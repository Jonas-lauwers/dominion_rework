...
public class cardConnection {	
	...
	/**public static ArrayList<String> getCardOfType(String expansionName, String type) {
		ResultSet resultSet = null;
		ArrayList<String> result = new ArrayList<>();
		try {
			String stmt = "SELECT Name, Cost, Text FROM dominioncards WHERE Expansion = ? AND Cardtype = ?";
			PreparedStatement preparedStatement = ConnectionManager.getInstance().getConnection().prepareStatement(stmt);
			preparedStatement.setObject(1, expansionName);
